  ...
  Future<void> _loadInitialPosts() async {
    ...
    try {
      ...
      if (_selectedFilter == 'For You') {
        ...
        newPosts = (await FirebaseFirestore.instance
      ...
      // Identify user IDs we don't have in cache yet
      ...
      for (var doc in newPosts) {
        ...
        if (data['userId'] != null && !_userCache.containsKey(data['userId'])) {
      ...
      // Fetch them in parallel (Future.wait) rather than one by one

      if (userIdsToFetch.isNotEmpty) {

        // Note: Firestore 'whereIn' is limited to 10 items. 

        // For simplicity/speed, we use individual fetches wrapped in Future.wait 

        // or you can implement chunking. Here is the fast parallel fetch:

        final userDocs = await Future.wait(

          userIdsToFetch.map((id) => FirebaseFirestore.instance.collection('users').doc(id).get())
